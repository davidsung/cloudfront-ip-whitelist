...
# 2. It has to be tagged 'Name: cloudfront_g_l' or 'Name: cloudfront_g_h' or 'Name: cloudfront_r'
# 3. It has to be tagged 'AutoUpdate: true'
# If any of these 3 are not true, the security group will be unmodified.
GLOBAL_L_SG_TAGS = { 'Name': 'cloudfront_g_l', 'AutoUpdate': 'true' }
GLOBAL_H_SG_TAGS = { 'Name': 'cloudfront_g_h', 'AutoUpdate': 'true' }
...
def lambda_handler(event, context):
    ...
    global_l_cf_ranges = get_ranges_for_service(ip_ranges, SERVICE, "GLOBAL_L")
    global_h_cf_ranges = get_ranges_for_service(ip_ranges, SERVICE, "GLOBAL_H")
    region_cf_ranges = get_ranges_for_service(ip_ranges, SERVICE, "REGION")

    # Update the security groups
    result = update_security_groups(global_l_cf_ranges, "GLOBAL_L")
    result = result + update_security_groups(global_h_cf_ranges, "GLOBAL_H")
...
def get_ranges_for_service(ranges, service, subset):
    ...
    for prefix in ranges['prefixes']:
        if prefix['service'] == service and (("GLOBAL" == prefix['region'] and (subset == "GLOBAL_H" or subset == "GLOBAL_L")) or ((subset != 'GLOBAL_H' or subset != 'GLOBAL_L') and prefix['region'] != 'GLOBAL')):
            logging.info(('Found ' + service + ' region: ' + prefix['region'] + ' range: ' + prefix['ip_prefix']))
            ip_int = int(ipaddress.IPv4Network(prefix['ip_prefix'])[0])
            if ("GLOBAL" == prefix['region'] and subset == 'GLOBAL_H' and ip_int > 2 ** 31) or ("GLOBAL" == prefix['region'] and subset == 'GLOBAL_L' and ip_int <= 2 ** 31) or (prefix['region'] != 'GLOBAL' and subset != 'GLOBAL_H' and subset != 'GLOBAL_L'):
                service_ranges.append(prefix['ip_prefix'])

    logging.info(f'Subset: {subset}, service_ranges length: {len(service_ranges)}')
...
def update_security_groups(new_ranges, rangeType):
    ...
    for curGroup in allSGs:
        tagToFind = {}
        if rangeType == "GLOBAL_L":
            tagToFind = GLOBAL_L_SG_TAGS
        elif rangeType == "GLOBAL_H":
            tagToFind = GLOBAL_H_SG_TAGS
        else:
